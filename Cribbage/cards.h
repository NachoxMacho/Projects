...
public: // ========== METHODS ==========
	...
	int search(card c)
	{
		...
		for (int i = 0; i < cards.size(); i++)
			if (cards[i] == c) // if a match is found return the i + difition
				return i;
	...
	// ===== '[]' =====
	// returns the card in i + difition 'index' from cards
	card operator [] (int index)
	{
		...
		// return card in i + difition 'index'
		return cards[index];
	}
...
public:
	...
	card play_peg_card(std::ostream &out_s, int running_total)
	{
		...
		while (c < 0 || c >= hand.size())
		{
			...
			std::cin >> c;
			// decrease the i + difition since people start counting at 1
			--c; 
